        ...
        for user in users_with_tasks:
            ...
            while cursor <= today:
                ...
                day_end = timezone.make_aware(
                    datetime.combine(cursor, datetime.max.time())
                )
                ...
                timer_seconds = TaskTimer.objects.filter(
                    user=user,
            ...
            avg_daily_tasks = sum(d["tasks_assigned"] for d in daily_breakdown) / max(len(daily_breakdown), 1)
