...
import { leadsApi, crmApi } from "@/shared/api/base";
...
export function DealPipelinePage() {
  ...
  useEffect(() => {
    ...
    const onDragEnd = () => { dragLeadRef.current = null; };
  ...
  const { data: forecast, isLoading: forecastLoading } = useQuery({
    queryKey: [QUERY_KEYS.LEADS, "forecast"],
    queryFn: () => crmApi.leads.forecast(),
  ...
  const moveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.LEADS, "forecast"] });
    },
  });
  ...
  const handleDrop = useCallback((e: React.DragEvent, toStage: string) => {
    ...
    if (!lead || lead.fromStage === toStage) return;
  ...
  const handleExport = async () => {
    try {
      ...
      const csv = [
        ["Контакт", "Компания", "Телефон", "Email", "Бюджет", "Этап", "Источник", "Создан"],
        ...data.map((l: Lead) => [
          l.contact_name,
          l.company_name,
          l.phone,
  ...
  if (kanbanLoading || forecastLoading) {
    return (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Сделки / Воронка"
        description="Этапы продаж с прогнозом и вероятностью закрытия"
        actions={
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card>
              <div className="flex items-center justify-between">
                <p className="text-sm text-surface-500">Воронка всего</p>
              ...
              <p className="mt-2 text-2xl font-bold text-surface-900 dark:text-white">
                {formatCurrency(forecast.total_pipeline)}
              </p>
              <p className="mt-1 text-xs text-surface-400">
                Ожидаемая выручка с учётом вероятностей
              </p>
            </Card>
            <Card>
              <div className="flex items-center justify-between">
                <p className="text-sm text-surface-500">Общий бюджет</p>
                <div className="rounded-lg bg-warning-50 p-2 dark:bg-yellow-900/20">
                  <TrendingUp className="h-4 w-4 text-warning-600" />
                </div>
              </div>
              <p className="mt-2 text-2xl font-bold text-surface-900 dark:text-white">
                {formatCurrency(forecast.total_budget)}
              </p>
              <p className="mt-1 text-xs text-surface-400">
                Сумма всех бюджетов в воронке
              </p>
            </Card>
            <Card>
              <div className="flex items-center justify-between">
                <p className="text-sm text-surface-500">Активные сделки</p>
                <div className="rounded-lg bg-success-50 p-2 dark:bg-green-900/20">
                  <Target className="h-4 w-4 text-success-600" />
                </div>
              </div>
              <p className="mt-2 text-2xl font-bold text-surface-900 dark:text-white">
                {forecast.total_leads}
              </p>
              <p className="mt-1 text-xs text-surface-400">
                Всего активных лидов
              </p>
            </Card>
            <Card>
              <div className="flex items-center justify-between">
                <p className="text-sm text-surface-500">Этапов</p>
                <div className="rounded-lg bg-surface-100 p-2 dark:bg-surface-700">
                  <Users className="h-4 w-4 text-purple-600" />
                </div>
              </div>
              <p className="mt-2 text-2xl font-bold text-surface-900 dark:text-white">
                {forecast.stages.length}
              </p>
              <p className="mt-1 text-xs text-surface-400">
                Этапов в воронке продаж
              </p>
            </Card>
          </div>
          ...
          <Card>
            ...
            <div className="space-y-3">
              {forecast.stages.map((stage) => {
                const maxBudget = Math.max(...forecast.stages.map((s) => s.total_budget), 1);
                const widthPct = (stage.total_budget / maxBudget) * 100;
                return (
                  <div key={stage.stage_id} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded bg-surface-100 px-1.5 py-0.5 text-xs text-surface-500 dark:bg-surface-700">
                          {stage.lead_count} лидов
                        </span>
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-surface-500">{formatCurrency(stage.total_budget)}</span>
                        <span className="font-semibold text-success-600">
                          {formatCurrency(stage.expected_revenue)}
                        </span>
                      </div>
                    </div>
                    <div className="relative h-2.5 rounded-full bg-surface-100 dark:bg-surface-700">
                      <div
      ...
      {view === "kanban" && (
        <div ref={scrollContainerRef} className="flex gap-4 overflow-x-auto pb-4">
          {kanbanData?.map((column) => {
            ...
            const stageForecast = forecast?.stages.find((s) => s.stage_id === column.id);
            return (
              <div
                key={column.id}
                className={cn(
                  "min-w-[320px] flex-shrink-0 rounded-xl transition-all",
              ...
              >
                <div className="mb-3 flex items-center justify-between px-3 pt-3">
                  ...
                  {stageForecast && stageForecast.total_budget > 0 && (
                    <span className="text-xs font-medium text-success-600">
                      {formatCurrency(stageForecast.expected_revenue)}
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-2 px-3 pb-3">
                  {(column.leads || []).length === 0 ? (
                    <div className="flex min-h-[100px] items-center justify-center rounded-lg border-2 border-dashed border-surface-200 dark:border-surface-700">
                      <p className="text-sm text-surface-400">Перетащите сделку сюда</p>
                    </div>
                  ) : (
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-surface-500 truncate">{lead.company_name || lead.phone}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {lead.budget && (
      ...
      {(!kanbanData || kanbanData.every((c) => c.leads.length === 0)) && view === "kanban" && (
        <EmptyState
          title="Нет сделок в воронке"
          description="Добавьте первую сделку, чтобы начать продажи"
          action={
            <Button onClick={() => setShowCreateModal(true)}>
              <Plus className="h-4 w-4" />
              Добавить сделку
      ...
      <Modal open={showCreateModal} onClose={() => setShowCreateModal(false)} title="Новая сделка">
        <LeadForm
          ...
          stages={kanbanData?.map((c) => ({ value: c.id, label: c.title })) || []}
        />
      </Modal>
...
function LeadCard({
  ...
  return (
    <div
      ...
      className="group/card cursor-grab rounded-lg border border-surface-200 bg-white p-3 transition-all hover:-translate-y-0.5 active:cursor-grabbing active:opacity-60 dark:border-surface-700 dark:bg-surface-800"
    >
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={3} className="input mt-1" />
      </div>
